// ===============================
// Anemometer 3D Sensor Monitoring
// ===============================
// SENSOR_ID injected by Flask template (anm3d.html)
const _sid3d = (typeof SENSOR_ID !== 'undefined' && SENSOR_ID) ? `&sensor_id=${SENSOR_ID}` : '';
const ANM3D_API = `/api/anm3d/latest?dummy=1${_sid3d}`;
const ANM3D_HISTORY_API = `/api/anm3d/history?dummy=1${_sid3d}`;
const ANM3D_TS_API = `/api/anm3d?dummy=1${_sid3d}`;
...
// ===============================
// amCharts 5 — ANM3D Chart
// ===============================
am5.ready(function () {
    const root = am5.Root.new("anm3d-chart");
    ...
    // 1. Elevation (kiri terluar) — auto-scale dengan 10% headroom
    const yAxisElev = SHMChart.createValueYAxis(chart, root, C, { opposite: false });
    const elevLabel = SHMChart.addYLabel(yAxisElev, root, 'Elevation (°)', C);
    // Grid horizontal dari axis paling kiri (elevation)

    // 2. Wind Speed (kiri dalam) — auto-scale dengan 10% headroom
    ...
    // 3. Direction (kanan) — tetap max 360° karena derajat kompas
    ...
    const dirSeries = makeLineSeries('Wind Direction', 'wind_direction', yAxisDir, 0xf59e0b);
    const elevSeries = makeLineSeries('Wind Elevation', 'wind_elevation', yAxisElev, 0x22c55e);
    ...
    const dirPulse = SHMChart.createPulseSeries(chart, root, xAxis, yAxisDir, 'wind_direction', 0xf59e0b);
    const elevPulse = SHMChart.createPulseSeries(chart, root, xAxis, yAxisElev, 'wind_elevation', 0x22c55e);
    ...
    var dirLabelSeries = createLabelSeries(yAxisDir, 0xf59e0b, 'wind_direction', "--°", -18);
    var elevLabelSeries = createLabelSeries(yAxisElev, 0x22c55e, 'wind_elevation', "--°", 32);
    ...
    function updateLatestTooltip(point) {
        ...
        if (point.wind_elevation != null) elevLabelSeries.data.setAll([{ time: point.time, wind_elevation: point.wind_elevation }]);
    }
    ...
    const anm3dWatcher = window.SHMToast
        ? window.SHMToast.watchSensor({ sensorName: `Anemometer 3D ${typeof SENSOR_ID !== 'undefined' ? SENSOR_ID : ''}`.trim(), timeoutMs: 120000 })
        : null;

    async function loadChartHistory() {
        try {
            const res = await fetch(ANM3D_TS_API + '&limit=10');
            ...
            chartData = rows.map(d => ({
                ...
                wind_elevation: d.wind_elevation,
            })).reverse();

            spdSeries.data.setAll(chartData);
            dirSeries.data.setAll(chartData);
            elevSeries.data.setAll(chartData);
            tooltipSeries.data.setAll(chartData);

            const latest = chartData[chartData.length - 1];
            spdPulse.data.setAll([latest]);
            dirPulse.data.setAll([latest]);
            elevPulse.data.setAll([latest]);
            updateLatestTooltip(latest);

            updateSummary(rows[0]);
        } catch (e) {
            console.warn('ANM3D history load error:', e);
            if (window.SHMToast) window.SHMToast.danger('Gagal memuat data riwayat Anemometer 3D', 'Anemometer 3D');
        }
    }

    // ── Socket.IO streaming ──
    socket.on('anm3d_update', (d) => {
    ...
    function updateTooltipText() {
        ...
        const dActive = !document.getElementById('legend-dir').classList.contains('inactive');
        const eActive = !document.getElementById('legend-elev').classList.contains('inactive');

        if (sActive) labelData.push("[bold]Speed:[/] {wind_speed}m/s");
        if (dActive) labelData.push("[bold]Direction:[/] {wind_direction}°");
        if (eActive) labelData.push("[bold]Elevation:[/] {wind_elevation}°");
    ...
    SHMChart.setupLegendToggle('legend-dir', dirSeries, dirPulse, yAxisDir, updateTooltipText);
    SHMChart.setupLegendToggle('legend-elev', elevSeries, elevPulse, yAxisElev, updateTooltipText);
    ...
    SHMChart.watchTheme(() => {
        SHMChart.refreshAxisColors([xAxis, yAxisElev, yAxisSpd, yAxisDir], [elevLabel, spdLabel, dirLabel]);
    });
});
...
function updateSummary(d) {
    ...
    const elevEl = document.getElementById('summary-wind-elevation');
    ...
    if (elevEl) elevEl.textContent = d.wind_elevation != null ? d.wind_elevation.toFixed(1) + ' °' : '--';
}

// ── Data Table ──
const tableBody = document.getElementById('anm3d-table-body');
...
function renderRealtimeTable() {
    ...
    realtimeTableData.forEach(d => {
        ...
        tr.innerHTML = `
            ...
            <td>${d.wind_elevation != null ? d.wind_elevation.toFixed(1) + '°' : '--'}</td>
            <td>${d.sensor_id || '--'}</td>
        `;
...
async function loadTableHistory() {
    try {
        const res = await fetch(ANM3D_HISTORY_API + '&limit=100');
        const rows = await res.json();
        if (!tableBody) return;
        realtimeTableData = rows;
        renderRealtimeTable();
    } catch (e) {
        console.warn('ANM3D history table error:', e);
    }
}
...
function initStatistikSearch() {
    ...
    if (optionSelect) {
        optionSelect.addEventListener('change', () => {
            ...
            const yAxisElev = chart.yAxes.getIndex(0);
            const yAxisSpd = chart.yAxes.getIndex(1);
            const yAxisDir = chart.yAxes.getIndex(2);

            const lblElev = yAxisElev?.get("customLabel");
            ...
            const elevLegends = [
                document.getElementById('legend-stat-avg-elev'),
                document.getElementById('legend-stat-max-elev'),
                document.getElementById('legend-stat-min-elev')
            ];
            ...
            if (val === 'semua') {
                anm3dSeriesRefs.speedSeries.forEach(s => s.show());
                anm3dSeriesRefs.dirSeries.forEach(s => s.show());
                anm3dSeriesRefs.elevSeries.forEach(s => s.show());
                ...
                if (yAxisElev) {
                    yAxisElev.show();
                    yAxisElev.set("visible", true);
                    if (lblElev) { lblElev.show(); lblElev.set("visible", true); }
                    const r = yAxisElev.get("renderer");
                    if (r) {
                        r.set("visible", true);
                        if (r?.gridContainer) r.gridContainer.set("visible", true);
                        if (r?.labelsContainer) r.labelsContainer.set("visible", true);
                ...
                toggleDisplay(elevLegends, '');
                [...spdLegends, ...dirLegends, ...elevLegends].forEach(el => {
                    if (el) el.classList.remove('inactive-legend');
                });
            } else if (val === 'ws') {
                anm3dSeriesRefs.speedSeries.forEach(s => s.show());
                anm3dSeriesRefs.dirSeries.forEach(s => s.hide());
                anm3dSeriesRefs.elevSeries.forEach(s => s.hide());
                ...
                if (yAxisElev) {
                    ...
                    if (lblElev) {
                        lblElev.hide();
                        lblElev.set("visible", false);
                        lblElev.set("opacity", 0);
                    }
                    const r = yAxisElev.get("renderer");
                    if (r) {
                        r.setAll({
                ...
                toggleDisplay(elevLegends, 'none');
                ...
                elevLegends.forEach(el => { if (el) el.classList.add('inactive-legend'); });
            } else if (val === 'wd') {
                anm3dSeriesRefs.speedSeries.forEach(s => s.hide());
                anm3dSeriesRefs.dirSeries.forEach(s => s.show());
                anm3dSeriesRefs.elevSeries.forEach(s => s.hide());

                if (yAxisSpd) {
                    yAxisSpd.hide();
                    yAxisSpd.set("visible", false);
                    if (lblSpd) { lblSpd.hide(); lblSpd.set("visible", false); }
                    const r = yAxisSpd.get("renderer");
                    if (r) {
                        r.set("visible", false);
                ...
                if (yAxisElev) {
                    yAxisElev.hide();
                    yAxisElev.set("visible", false);
                    if (lblElev) { lblElev.hide(); lblElev.set("visible", false); }
                    const r = yAxisElev.get("renderer");
                    if (r) {
                        r.set("visible", false);
                        if (r?.gridContainer) r.gridContainer.set("visible", false);
                ...
                if (yAxisDir) {
                    yAxisDir.hide();
                    yAxisDir.set("visible", false);
                    if (lblDir) { lblDir.hide(); lblDir.set("visible", false); }
                    const r = yAxisDir.get("renderer");
                    if (r) {
                        r.set("visible", false);
                        if (r?.gridContainer) r.gridContainer.set("visible", false);
                ...
                spdLegends.forEach(el => { if (el) el.classList.add('inactive-legend'); });
                dirLegends.forEach(el => { if (el) el.classList.add('inactive-legend'); });
                elevLegends.forEach(el => { if (el) el.classList.remove('inactive-legend'); });
            }
        });
    }
}
...
async function loadStatistikByRange(start, end) {
    try {
        ...
        const res = await fetch(`/api/anm3d/statistik/range?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}${sidUrl}`);
        const data = await res.json();

        const legendCont = document.getElementById('anm3d-statistik-legend');
        const summaryCont = document.getElementById('anm3d-statistik-summary');

        if (!data.length) {
            if (window.SHMToast) window.SHMToast.warning("Data statistik tidak ditemukan untuk periode ini", "Statistik");

            // Clear summary
            const sumIds = ['stat-summary-min-speed', 'stat-summary-max-speed', 'stat-summary-avg-speed', 'stat-summary-avg-direction', 'stat-summary-avg-elevation'];
            sumIds.forEach(id => {
            ...
            // Clear table
            const tbody = document.getElementById('anm3d-statistik-table-body');
        ...
        document.getElementById('stat-summary-avg-elevation').textContent = first.avg_wind_elevation?.toFixed(1) || '--';
...
function populateStatistikTable(data) {
    ...
    // Reverse to show newest data first (Descending)
    ...
    reversedData.forEach(d => {
        ...
        tr.innerHTML = `
            ...
            <td>${d.min_wind_elevation != null ? d.min_wind_elevation.toFixed(2) : '--'}</td>
            <td>${d.max_wind_elevation != null ? d.max_wind_elevation.toFixed(2) : '--'}</td>
            <td>${d.avg_wind_elevation != null ? d.avg_wind_elevation.toFixed(2) : '--'}</td>
            <td>${d.th1 != null ? Number(d.th1).toFixed(2) : '--'}</td>
            <td>${d.th2 != null ? Number(d.th2).toFixed(2) : '--'}</td>
        `;
...
function renderStatistikChart(data) {
    ...
    statRoot = am5.Root.new("anm3d-stat-chart");
    ...
    // 1. Elevation (kiri terluar)
    const yAxisElev = SHMChart.createValueYAxis(chart, statRoot, C, { opposite: false, min: -60, max: 60 });
    const lblElev = SHMChart.addYLabel(yAxisElev, statRoot, 'Wind Elevation (°)', C);
    yAxisElev.set("customLabel", lblElev);

    // 2. Speed Axis (kiri dalam)
    ...
    const chartData = data.map(d => ({
        ...
        min_we: d.min_wind_elevation,
        max_we: d.max_wind_elevation,
        avg_we: d.avg_wind_elevation
    }));
    ...
    // Direction (0xf59e0b orange, 0x10b981 green, 0xec4899 pink)
    const s4 = addSeries("Avg Wind Direction", "avg_wd", yAxisDir, 0xf59e0b);
    ...
    // Elevation (0x6366f1 indigo, 0x8b5cf6 violet, 0xd946ef magenta)
    const s7 = addSeries("Avg Wind Elevation", "avg_we", yAxisElev, 0x6366f1);
    const s8 = addSeries("Max Wind Elevation", "max_we", yAxisElev, 0x8b5cf6);
    const s9 = addSeries("Min Wind Elevation", "min_we", yAxisElev, 0xd946ef);

    // Store series references for filtering
    anm3dSeriesRefs.speedSeries = [s1, s2, s3];
    anm3dSeriesRefs.dirSeries = [s4, s5, s6];
    anm3dSeriesRefs.elevSeries = [s7, s8, s9];
    ...
    toggle('legend-stat-avg-elev', s7, elevSeries, yAxisElev, lblElev);
    toggle('legend-stat-max-elev', s8, elevSeries, yAxisElev, lblElev);
    toggle('legend-stat-min-elev', s9, elevSeries, yAxisElev, lblElev);
    ...
    SHMChart.watchTheme(() => {
        SHMChart.refreshAxisColors([xAxis, yAxisElev, yAxisSpd, yAxisDir], [lblElev, lblSpd, lblDir]);
    });
...
window.captureAnm3DStatistik = function () {
    const target = document.getElementById("anm3dStatistikCardArea");
    if (!target) return;
    ...
    html2canvas(target, {
        ...
        link.download = `Anm3D_Statistik_${sid}_${new Date().toISOString().slice(0, 10)}.png`;
...
document.getElementById('btn-export-statistik-csv')?.addEventListener('click', () => {
    const rows = [...document.querySelectorAll('#anm3d-statistik-table-body tr')];
    const csv = ['Datetime,Sensor ID,Min WS,Max WS,Avg WS,Min WD,Max WD,Avg WD,Min WE,Max WE,Avg WE'];
    rows.forEach(tr => {
    ...
    a.download = `anm3d_statistik_${SENSOR_ID}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
});
...
document.getElementById('btn-export-statistik-pdf')?.addEventListener('click', () => {
    ...
    doc.setFontSize(14);
    doc.text(`Anemometer 3D Statistik Data – ${SENSOR_ID || 'All Sensors'}`, 14, 15);
    doc.setFontSize(9);
    ...
    doc.autoTable({
        startY: 27,
        head: [['Datetime', 'Sensor ID', 'Min WS', 'Max WS', 'Avg WS', 'Min WD', 'Max WD', 'Avg WD', 'Min WE', 'Max WE', 'Avg WE']],
        body: tableData,
        theme: 'striped',
    ...
    doc.save(`anm3d_statistik_${SENSOR_ID || 'all'}.pdf`);
});

// Export PDF Realtime
document.getElementById('btn-export-pdf')?.addEventListener('click', () => {
    const rows = [...document.querySelectorAll('#anm3d-table-body tr')];
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ orientation: 'landscape' });

    doc.setFontSize(14);
    doc.text(`Anemometer 3D Real-Time Data – ${SENSOR_ID || 'All Sensors'}`, 14, 15);
    doc.setFontSize(9);
    doc.setTextColor(100);
    doc.text(`Exported at: ${new Date().toLocaleString('id-ID')}`, 14, 22);
    ...
    doc.autoTable({
        startY: 27,
        head: [['Datetime', 'Wind Speed (m/s)', 'Wind Direction (°)', 'Wind Elevation (°)', 'Sensor ID']],
        body: tableData,
        theme: 'striped',
        headStyles: { fillColor: [59, 130, 246] },
        styles: { fontSize: 8 }
    ...
    doc.save(`anm3d_${SENSOR_ID || 'all'}.pdf`);
});

// Export CSV
document.getElementById('btn-export-csv')?.addEventListener('click', () => {
    const rows = [...document.querySelectorAll('#anm3d-table-body tr')];
    const csv = ['Datetime,Wind Speed (m/s),Wind Direction (°),Wind Elevation (°),Sensor ID'];
    rows.forEach(tr => {
        const cells = [...tr.querySelectorAll('td')].map(td => td.textContent.trim());
        csv.push(cells.join(','));
    });
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `anm3d_${SENSOR_ID}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
});


window.captureAnm3D = function () {
    const target = document.getElementById("anm3dCardArea");
    if (!target) return;

    if (typeof html2canvas === 'undefined') {
        console.error("html2canvas is not loaded");
        if (window.SHMToast) window.SHMToast.danger('Gagal menangkap gambar: Library tidak ditemukan', 'Anemometer 3D');
        return;
    }
    ...
    html2canvas(target, {
        ...
        const sid = typeof SENSOR_ID !== 'undefined' ? SENSOR_ID : 'Data';
        link.download = `Anemometer_3D_${sid}_${dateStr}.png`;
        link.href = canvas.toDataURL("image/png");
        link.click();
    }).catch(err => {
        console.error("Capture captureAnm3D error:", err);
